...
- R2：新增 `RocksDBStoreEngine.scanByPrefix(seekKey, prefix, limit, filter, consumer)` 和 `ScanStats`；`RocksDBIndexManager.scanXidsByStatus(status, minBeginTimeInclusive, maxBeginTimeInclusive, cursor, limit)` 已使用 status | beginTime lower-bound seek 和 cursor 续扫；`RocksDBTransactionStoreManager.readByStatuses` 在 `overTimeAliveMills` 或 `SessionCondition.limit` 存在时按 cursor 分页读取，避免先全量扫描再在 JVM 层截断；无 `limit` 且无 `overTimeAliveMills` 的 status 查询保留 `scanXidsByStatus(status, consumer)` single-iterator fast path；bounded RocksDB status 查询会通过 `SessionCondition.getScanStats()` 回填 rowsScanned/rowsReturned/pointReads/sessionsReturned/elapsedMillis/limitReached；`SessionCondition` 已携带 status scan cursor，coordinator 后台任务按 `GlobalStatus` 维护跨轮 cursor，避免每轮都从第一页开始；`timeoutCheck` 新增 deadline-aware timeout index 和 timeout scan cursor，只扫描已过期 Begin 候选页。
...
- R7：benchmark 增强了 tuning profile、WAL sync 对比、RocksDB 配置摘要和 WAL sync 统计输出；本轮新增 `queryIterationsPerRound`、`queryLimit`、`repeatRuns`、`compareOrder`，CSV 增加 repeat/order/query limit 与 `rowsScanned` / `rowsReturned` / `rowsUpdated` / `pointReads` / `iteratorNext` / `writeBatchBytes` / `innerOperations` 解释性列；query status 场景已接入 `queryLimit`，并可在 `expiredRatio > 0` 时走 overtime bounded scan；bounded `query.status` 已使用 `SessionCondition.getScanStats()` 回填 rowsScanned/rowsReturned/pointReads，避免限量/overtime scan 指标偏乐观；expiredRatio 的 active 样本已放到更远的 future window，避免长跑中漂移成 expired；A/B comparison 的 ops/s 解析已改为按 CSV header 定位列，避免新增解释性列后读错下标；跨 repeat summary CSV/JSON、`statusDistribution`、`expiredRatio`、`lockWorkload`、`lockConflictRatio`、`xidFanoutDistribution` 已接入。
...
结果：exit 0；85 tests, 0 failures, 0 errors, 0 skipped。测试日志仍有既有的 SpringBootConfigurationProvider / SLF4J 噪声。

本轮补充验证：

```powershell
.\mvnw.cmd -pl server "-DskipITs=true" "-Dcheckstyle.skip=true" "-Dlicense.skip=true" "-Dspotless.check.skip=true" "-Djacoco.skip=true" "-Dtest=DefaultCoordinatorTest#retryBackgroundTasksSetSessionQueryLimit+retryBackgroundTasksCarryStatusScanCursorAcrossRounds+multiStatusBackgroundScanKeepsEachStatusBoundedPage+scheduledBackgroundTasksSetSessionQueryLimit" test
```

结果：exit 0；4 tests, 0 failures, 0 errors, 0 skipped。该测试类日志仍有既有的 Windows 文件锁清理噪声。

本轮追加验证：
```powershell
.\mvnw.cmd -pl server -DskipITs "-Dtest=RocksDBTransactionStoreManagerTest#testReadByTimeoutDeadlineUsesDeadlineOrderAndCursor+testGlobalUpdateRemovesTimeoutIndexForNonBeginStatus" test
.\mvnw.cmd -pl server -DskipITs "-Dtest=DefaultCoordinatorTest#timeoutCheckUsesDeadlineBoundedScanForRocksDBSessionManager+timeoutCheckKeepsFullScanForGenericSessionManager+timeoutCheckCarriesTimeoutScanCursorAcrossRounds" test
```

结果：两个命令均 exit 0；store 层 2 tests, coordinator 层 3 tests，均 0 failures / 0 errors / 0 skipped。`DefaultCoordinatorTest` 的后续 surefire execution 仍会输出既有 Windows 文件锁清理噪声，但命令退出码为 0。
...
- 恢复复杂度：R1/R2/R3 已把最危险的启动全量 orphan cleanup、后台 status 查询全量扫描和大 fanout lock release/update 做了首轮 bounded/streaming 化；当前风险从“单次无界阻塞”下降为“后台任务调度、进度、限速、观测还未产品化”。
...
- benchmark：R7 已能输出 repeat/order/query limit、rows scanned/returned/updated、pointReads、iteratorNext、writeBatchBytes、innerOperations 和 WAL sync 指标；bounded `query.status` 已使用 store scan stats 回填解释性指标；A/B ops/s 解析已随 CSV header 对齐，跨 repeat summary CSV/JSON 已输出 mean/median/p95/p99/min/max/stddev；`statusDistribution`、`expiredRatio`、`lockWorkload`、`lockConflictRatio`、`xidFanoutDistribution` 均可配置，expiredRatio active 样本已避免长跑漂移，lock acquire/conflict/update/release/orphan clean 已具备基于 lock key 数量的 rows scanned/updated 解释口径。剩余不足是 RocksDB 内部 perf counter / stats snapshot 还没有和每个 scenario 直接关联。
- 当前进入阶段：已从“验证可行性和修明显 O(N) 风险”进入“性能路径补强 + 后台任务产品化 + 运维调参矩阵 + crash-injection/RPO 验证”的阶段；短期执行顺序先补性能路径，RocksDB 内部 stats/perf snapshot 和告警收口后置。
...
- 当前进展：retry rollbacking、retry committing、async committing、rollbacking/committing scheduled、end-state scheduled 已显式传入 `SessionCondition.limit`；coordinator 侧多状态任务先拆成单状态 bounded 查询，再按 beginTime 排序合并每个 status 的 bounded page，不再二次截断导致 cursor 已推进但 session 未处理；RocksDB store 层 `readByStatuses` 已支持 bounded 多状态 k-way merge，按 beginTime 全局有序读取并校验 stale status/beginTime index，不再按 status 顺序拼接导致单个状态占满本轮结果；无 `limit` 且无 `overTimeAliveMills` 的 status 查询保留 single-iterator fast path；bounded status 查询已回填 `SessionCondition.getScanStats()`，后台任务会记录每个 status 子查询的 rowsScanned/rowsReturned/pointReads/limitReached 和耗时；`timeoutCheck` 已改为 RocksDB 专属 deadline-aware bounded scan，使用 `beginTime + timeout` 有序索引避免长 timeout 的早期事务阻塞后续短 timeout 事务。
- 主要任务：
  ...
  - 已完成：`timeoutCheck` 不直接限制最早 beginTime 前 N 条，而是新增 `GLOBAL_TIMEOUT_INDEX`，按 `beginTime + timeout` deadline 有序扫描；`SessionCondition` 携带 timeout deadline 和 timeout scan cursor，coordinator 仅在 `RocksDBSessionManager` 下传入 batch limit，普通 session manager 仍保留全量 Begin 扫描语义。
  - 已完成：bounded RocksDB status 查询回填每轮 rowsScanned、rowsReturned、pointReads、sessionsReturned、耗时和 limitReached；coordinator 后台任务按 status 子查询输出 debug 级扫描统计。
  - 已完成：store 层 bounded 多状态 k-way merge，复用每个 status 的 cursor/page scan，并按 beginTime/status/xid 稳定合并。
  - 已完成：无 `limit` 且无 `overTimeAliveMills` 的 status 查询保留 single-iterator fast path，避免全量状态查询被强制拆成多轮 page scan。
  - 已完成：跨轮 cursor merge 防饥饿/公平性；`SessionCondition` 携带 status scan cursor，RocksDB 单状态 bounded scan 返回 next cursor，coordinator 按 `GlobalStatus` 维护跨轮 cursor，扫到尾或空结果后回卷。
  - 待完成：专项 benchmark 对比。
- 验收：
...
- 主要任务：
  ...
  - 已完成：bounded `query.status` 使用 `SessionCondition.getScanStats()` 回填真实 rowsScanned/rowsReturned/pointReads；无界 fast path 保留估算口径。
  - 已完成：expiredRatio 的 active 样本 beginTime 放到更远的 future window，避免大规模/长时间 benchmark 中 active 样本漂移为 expired。
  ...
  - 已完成：CSV 和 summary CSV/JSON 输出 benchmark 级 `pointReads`、`iteratorNext`、`writeBatchBytes` 估算，用于解释点查、迭代扫描和写批 payload 规模。
  - 待完成：继续接入 RocksDB 内部 stats/perf snapshot，把 block cache、iterator、write stall 等底层计数与每个 scenario 关联。
...
- 已完成：`RocksDBTransactionStoreManager.readByStatuses` 在 `SessionCondition.overTimeAliveMills > 0` 或 `SessionCondition.limit > 0` 时按 cursor 分页读取，并支持限制本轮返回数量。
- 已完成：bounded status-only 路径走 paged status index scan，不再先扫描状态全集再在 JVM 层截断；无 `limit` 且无 `overTimeAliveMills` 的 status 查询保留 single-iterator fast path；query benchmark 的 `queryLimit` 因此可以触发真实限量 scan 路径。
- 已完成：retry/end-state 相关后台任务已显式传入 `SessionCondition.limit`，并通过 coordinator 侧单状态 bounded fan-in 合并每个 status 的 bounded page，避免多状态任务被当前按 status 顺序拼接的实现占满本轮 batch。
- 已完成：RocksDB store 层 bounded 多状态 `readByStatuses` 已使用 k-way merge，按 beginTime 全局有序返回，并跳过 stale status/beginTime index 对排序和去重的影响。
- 已完成：bounded RocksDB status 查询会通过 `SessionCondition.getScanStats()` 回填 rowsScanned/rowsReturned/pointReads/sessionsReturned/elapsedMillis/limitReached，coordinator 后台任务已接入 debug 级扫描统计日志。
- 已完成：`timeoutCheck` 已接入 deadline-aware bounded scan；新增 `GLOBAL_TIMEOUT_INDEX` 按 `beginTime + timeout` 排序，RocksDB store 通过 `SessionCondition.maxTimeoutDeadlineMillis` 和 timeout cursor 分页读取过期 Begin 候选，并继续用 `globalSession.isTimeout()` 做最终判定。
- 待完成：专项 benchmark 对比。
...
2. 改造后台任务：
   - timeout check 只扫描 `Begin` 且 `beginTime + timeout < now` 的前 N 条候选。
...
- 已完成：benchmark 主流程 CSV 已输出 `queryIterationsPerRound`、`queryLimit`、`repeatRun`、`compareOrder`、`rowsScanned`、`rowsReturned`、`rowsUpdated`、`pointReads`、`iteratorNext`、`writeBatchBytes`、`innerOperations`；compare 支持 `repeatRuns` 和 `compareOrder=AB|BA|ABBA` 等顺序控制。
...
- 已完成：新增 `expiredRatio`、`lockWorkload`、`lockConflictRatio`、`xidFanoutDistribution`，覆盖 overtime status query、lock 子 workload、冲突比例和可变 branch fanout；expiredRatio active 样本已放到更远 future window，避免长跑漂移；lock 相关场景已补 `rowsScanned` / `rowsUpdated` 解释口径。
- 已完成：新增 benchmark 级 `pointReads`、`iteratorNext`、`writeBatchBytes` 估算列，并在 summary CSV/JSON 中聚合输出；bounded `query.status` 已用 store scan stats 回填 rowsScanned/rowsReturned/pointReads。
- 待完成：RocksDB 内部 stats/perf snapshot 与每个 scenario 的直接关联仍需继续增强。
...
4. 增加解释性指标：
   ...
   - `rowsReturned`
   - `rowsUpdated`
   - `pointReads`（已完成 benchmark 级估算）
   - `iteratorNext`（已完成 benchmark 级估算）
   - `writeBatchBytes`（已完成 benchmark 级估算）
   - `rocksdbStatsSnapshot`（待完成内部 stats/perf 采样）
...
1. R2：status + beginTime bounded scan 已完成 lower-bound/cursor API，`overTimeAliveMills` 或 `limit` 读取路径已按 cursor 分页，无界 status 查询保留 single-iterator fast path；retry/end-state 后台任务已显式传入 batch limit，coordinator 侧保留单状态 bounded fan-in 并合并每个 status 的 bounded page，RocksDB store 层已支持 bounded 多状态 k-way merge 并跳过 stale status/beginTime index，后台任务基础 scan stats/耗时日志已接入；跨轮 cursor merge 已接入，避免 retry/end-state 持续失败时每轮卡在第一页；`timeoutCheck` 已接入 deadline-aware timeout index bounded scan；下一步补专项 benchmark。
